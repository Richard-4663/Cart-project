...
export default function PriceSection({ produto }) {
    ...
    const handleAddToCart = () => {
        ...
        AddItemCart(produtoParaCarrinho);
    };
    ...
    return(
        <div className='p-3 rounded-3 price-section'>
            <div className="product-price text-white">
                ...
                <h1>R$ {Number(produto?.preco || 0).toFixed(2)}</h1>
            ...
            <div className="price-fixed z-2">
                <div className={`d-flex ${largura >= 668 && 'gap-3'}`}>
                    <button 
                        className={`btn-buy w-50 p-3 ${largura <= 668 && "rounded-0"}`}
                        onClick={() => {
                            handleAddToCart();
                            navigate('/cart');
                        }}
                    ...
                    <button 
                        className={`btn-add w-50 p-3 ${largura <= 668 && "rounded-0"}`} 
            ...
            <div className='mt-4'>
                ...
                <form onSubmit={handleSubmit} className='d-flex justify-content-center frete'>
                    <input 
                        className="form-control w-50 me-3" 
                        type="text" 
                        value={formatarCepExibicao(CEP)} 
                        onChange={(e) => setCEP(e.target.value.replace(/\D/g, ''))} 
                        onFocus={() => setPlaceholder('_____-___')} 
                        onBlur={() => setPlaceholder("Inserir CEP")} 
                        placeholder={placeholder} 
                        maxLength={9}
                    />
                    <button type="submit" className="btn-add w-25">
                ...
                {exibirCEP && !erroCEP && ( 
                    <div className="mt-3">
                        ...
                        <p className="text-white-50">
                            {exibirCEP.street || 'Rua não encontrada'}, 
                            {exibirCEP.neighborhood || 'Bairro não encontrado'} - 
                            {exibirCEP.city || 'Cidade não encontrada'}, 
                            {exibirCEP.state || 'Estado não encontrado'}
                        </p>
                    </div>
                )}
            </div>
